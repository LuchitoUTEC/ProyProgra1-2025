...
def produccion_producir(estado):
    if estado.get("Prohibir Produccion", False) or estado.get("TurnosSinProduccionDerrame", 0) > 0 or estado.get(
            "TurnosSinOperacionesHuelgaNacional", 0) > 0:
...
def compras_comprar_insumos_nacionales(estado):
    costo = 10000 * estado["DescuentoCompraNacional"]
